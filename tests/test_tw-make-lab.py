...
def test_make_custom_lab_game():
    ...
    with make_temp_directory(prefix="test_tw-make-lab") as tmpdir:
        ...
        command = ["../scripts/tw-make-lab", "custom", "--seed", seed , "--output", game_file, "--surface_mode", "medium", "--merge_serial_actions", "--merge_parallel_actions", "--max_quest_length", "13", "--max_search_steps", "1000", "--lab_config_path", str(fpath)]
        print(' '.join(command))
        try:
...
def test_make_lab_game_from_level():
    ...
    with make_temp_directory(prefix="test_tw-make-lab") as tmpdir:
        ...
        command = ["../scripts/tw-make-lab", "challenge", "--seed", seed , "--output", game_file, "--surface_mode", "medium", "--merge_serial_actions", "--merge_parallel_actions", "--challenge", "tw-lab_game-level%d" % (level)]
        print(' '.join(command))
        try:
            assert check_call(command) == 0
        except CalledProcessError as e:
