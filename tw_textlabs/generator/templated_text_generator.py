...
import re
from tw_textlabs.generator.token_generators import TokenGenerator, grab_all
...
class TemplatedTextGenerator(object):
    ...
    def register_token_generator(self, tg, override=False):
        """ Register a token generator after verifying it's valid. """
        if (tg.symbol() in self.token_generators_dict) and (not override):
    ...
    def generate_entity(self, entity_type: str, internal_name: str = None) -> str:
        ...
        if entity_type in self.token_generators_dict:
            entity_name = self.token_generators_dict[entity_type].generate(
