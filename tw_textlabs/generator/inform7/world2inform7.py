...
from tw_textlabs.utils import make_temp_directory, str2bool

from tw_textlabs.generator.data import KnowledgeBase
from tw_textlabs.generator.game import Game, Quest
from tw_textlabs.generator.world import WorldRoom, WorldEntity
from tw_textlabs.logic import Signature, Proposition, Action


I7_DEFAULT_PATH = resource_filename(Requirement.parse('tw_textlabs'), 'tw_textlabs/thirdparty/inform7-6M62')
...
class Inform7Game:
    ...
    def gen_source(self, seed: int = 1234) -> str:
        ...
        # List the state of a material when examined
        source += textwrap.dedent("""\
        After examining a noun:
            if the noun provides the property state:
                say "The [noun] is in state [the state of the noun]"
        ...
        Instead of examining a noun when the noun is a mixture:
            say "The [noun] is in state [the state of the noun]. ";
            if a composition is part of the noun:
                let the current composition be a random composition which is part of the noun;
                say "The mixture is composed of [a list of things in the current composition]. "
        ...
        The taking allowed flag is usually true.
