...
def register_games(game_files: List[str],
    ...
    Example:
        ...
        >>> from tw_textlabs.generator import make_game, compile_game
        ...
        >>> env_id = tw_textlabs.gym.register_games([game_file], request_infos)
        >>> env = gym.make(env_id)
        >>> ob, infos = env.reset()
    ...
    register(
        id=env_id,
        entry_point='tw_textlabs.gym.envs:TextworldGamesEnv',
...
def register_game(game_file: str,
    ...
    Example:
        ...
        >>> from tw_textlabs import EnvInfos
        >>> request_infos = EnvInfos(description=True, inventory=True, extras=["more"])
        >>> env_id = tw_textlabs.gym.register_game(game_file, request_infos)
        >>> env = gym.make(env_id)
        >>> ob, infos = env.reset()
        >>> print(infos["extra.more"])
        This is extra information.
...
def make_batch(env_id: str, batch_size: int, parallel: bool = False) -> str:
    ...
    entry_point = 'tw_textlabs.gym.envs:BatchEnv'
    if parallel and batch_size > 1:
        entry_point = 'tw_textlabs.gym.envs:ParallelBatchEnv'
